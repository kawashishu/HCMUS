...
"""
================================================================================
MYFS DAT - PHIÊN BẢN TÁI CẤU TRÚC VỚI CƠ CHẾ LOGIN / LOGOUT & INTERACTIVE SHELL
--------------------------------------------------------------------------------
1/ Chạy "python myfs.py create" để khởi tạo/định dạng MyFS.dat (chỉ cần 1 lần).
2/ Chạy "python myfs.py login" để đăng nhập vào MyFS:
   - Kiểm tra code integrity
   - Kiểm tra OTP động
   - Kiểm tra mật khẩu MyFS (nếu MyFS đã có mật khẩu)
   => Nếu thành công, chương trình bước vào giao diện dòng lệnh (shell) tương tác.
3/ Trong giao diện shell này, các lệnh sẽ hoạt động không cần nhập lại
   mật khẩu MyFS:
   - set-pw, list, import, export, delete,
     list-trash, restore, set-file-pw, backup, restore-backup
4/ Gõ "logout" (hoặc Ctrl+C, hoặc Ctrl+D) để thoát phiên đăng nhập.
5/ Chương trình tự động dừng. Muốn login lại, phải chạy "python myfs.py login" lần nữa.
6/ Đã bổ sung try/except để tránh chương trình sập khi user nhập sai tham số.
================================================================================
"""
